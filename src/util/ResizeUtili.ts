...
export class ImageReSizeClass {
  ...
  async resizeImage(inputImgFile: File, imgWidth: number) {
    ...
    const sendFile: Promise<File | undefined | void> = new Promise(
        ...
        imageCrop.onload = () => {
          ...
          if (ctx !== null) {
            ...
            canvasElement.toBlob((file: Blob | null) => {
              if (file) {
                const resizeNewFile: File = new File(
                  [file],
                  "newfileName.jpg",
                  { lastModified: new Date().getTime(), type: file.type }
                );
...
export async function imageUpdateFireStorage(imageId: string, imageFile: File) {
  ...
  const inputImgFile2: File | undefined | void = await resizeClass.resizeImage(
    inputImgFile,
    imgSize
  );
  ...
  if (inputImgFile2 !== undefined) {
    ...
    await firebaseStorage
      .ref()
      .child("image/" + imageId + imgsoezi)
      .put(inputImgFile2, metadata)
      .then((docRef) => {
